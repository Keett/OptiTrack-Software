...
//== Camera SDK Sample
//==
//== This sample application assumes you have active marker hardware.  It will setup
//== the OptiTrack eSync with the appropriate settings and attach the active marker
//== labeling module to the camera.  from there this sample will display all markers
//== in the camera's field of view and also display the active tag or 'Passive' for
//== passive markers
//=================================================================================-----
...
const int kPassiveMarkerLabel = 0;
...
int main(int argc, char* argv[])
{
	...
	cModuleActiveLabel * activeLabeler = cModuleActiveLabel::Create();
    ...
    while(1)
    {
        ...
        if(frame)
        {
            ...
            for( int index=0; index<frame->ObjectCount(); index++ )
            {
                cObject * centroid = frame->Object(index);
                
                int label = centroid->Label();

                if( !centroid->IsFlag( cObject::Acquiring ) )
                {
                    if( label==kPassiveMarkerLabel )
                    {
                        sprintf_s( string, 100, "Passive" );
                    }
                    else
                    {
                        sprintf_s( string, 100, "Active #%d", label );
                    }

                    framebuffer->PrintMedium( (int) (centroid->X()+10), (int) (centroid->Y()-4), string );
                }
            }
	...
	//== Destroy active labeler ==--

    camera->RemoveModule( activeLabeler );
	cModuleActiveLabel::Destroy( activeLabeler );
